 ...
 *
 * 声明在 plannodes.h 中的大多数计划节点类型在这里都有对应的执行状态节点类型。
 * 一个例外是表达式节点（Expr 的子类型）通常由 ExprState 的步骤表示，
 * 并在 execExpr* 中完全处理——但有时它们的状态需要与执行器的其他部分共享，
 * 例如 SubPlanState，nodeSubplan.c 必须对其进行修改。
 *
 ...
 *
 * 在此文件中声明的节点类型没有任何 copy/equal/out/read 支持。
 * （目前这是在 gen_node_support.pl 中硬编码的，而不是在这里通过 pg_node_attr
 * 装饰显式表示。）
 *
 * There is no need for copy, equal, or read support for executor trees.
 *
 * 执行树不需要 copy、equal 或 read 支持。
 *
 ...
 * not provided.
 *
 * 输出支持对于调试可能有用；但有很多专业字段需要自定义代码，所以现在不提供。
 *
 *
...
/*
 * PostgreSQL 执行器状态节点系统核心流程说明：
 *
 * 执行器（Executor）的工作基于一组相互关联的状态节点（State Nodes），
 * 这些节点在 ExecutorStart 阶段根据计划树（Plan Tree）构建，并在 ExecutorRun
 * 阶段被递归处理。
 *
 * 核心流程如下：
 * 1. EState (Executor State):
 *    整个执行过程的总状态。它包含了查询范围表、全局内存上下文、参数信息以及所有的
 * TupleTableSlot。 它是执行器调度的上下文。
 *
 * 2. PlanState 及其子类 (如 ScanState, JoinState, AggState):
 *    对应于计划树中的每一个算子。每个 PlanState
 * 节点存储了该算子执行时的中间状态。 例如，ScanState
 * 存储了当前扫描的关系和扫描位置，HashState 存储了哈希表。
 *
 * 3. ExprContext:
 *    表达式执行上下文。它提供了评估表达式所需的“环境”，主要包括来自内部/外部/扫描计划的元组（TupleTableSlot）。
 *    它还管理着每元组（per-tuple）的内存上下文，用于防止内存泄漏。
 *
 * 4. ExprState:
 *    表示一个评估过的表达式树。它将复杂的表达式转换为一系列简单的执行步骤（ExprEvalStep），
 *    从而提高执行效率。
 *
 * 执行模式：
 * 采用的是需求驱动的拉取模式（Demand-pull
 * pipeline）。顶层节点通过递归调用下层节点的 ExecProcNode
 * 来获取元组，直到子树返回空，表示扫描结束。
 */
 ...
 * ExprState 代表整个表达式树的求值状态。
 * It contains instructions (in ->steps) to evaluate the expression.
 * 它包含求值表达式的指令（在 ->steps 中）。
 * ----------------
 */
typedef Datum (*ExprStateEvalFunc)(struct ExprState *expression,
                                   struct ExprContext *econtext, bool *isNull);
 ...
 *      此结构持有为特定索引构建新索引项所需的信息。
 *      用于索引构建 (index_build) 和单个索引项的创建。
 *
 *		NumIndexAttrs		此索引中的列总数
 *		NumIndexKeyAttrs	索引中的键列数
 *		IndexAttrNumbers
 *用作键的底层关系属性编号（零表示表达式）。 它还包含有关包含列的信息。
 *		Expressions			表达式项的表达式树，如果没有则为
 *NIL ExpressionsState	表达式的执行状态，如果没有则为 NIL Predicate
 *部分索引谓词，如果没有则为 NIL PredicateState
 *谓词的执行状态，如果没有则为 NIL ExclusionOps
 *每列的排他操作符，如果没有则为 NULL ExclusionProcs		ExclusionOps
 *的底层函数 OID ExclusionStrats		ExclusionOps 的操作族策略号
 *		UniqueOps			这些类似于
 *Exclusion*，但用于唯一索引 UniqueProcs UniqueStrats Unique
 *是否为唯一索引？ NullsNotDistinct	是否为 NULLS NOT DISTINCT？
 *		ReadyForInserts		是否可以进行插入？
 *		CheckedUnchanged	IndexUnchanged 状态是否已确定？
 *		IndexUnchanged		aminsert 提示，为零售插入缓存
 *		Concurrent			是否正在进行并行索引构建？
 *		BrokenHotChain		是否检测到任何断裂的 HOT 链？
 *		WithoutOverlaps		是否为 WITHOUT OVERLAPS 索引？
 *		Summarizing			是否为汇总索引？
 *		ParallelWorkers		请求的工作者数量（不包括 leader）
 *		Am					索引访问方法（AM）的 OID
 ...
 *		context.
 *      此类持有求值表达式进行元组限制（qualifications）和元组投影（projections）
 *      所需的“当前上下文”信息。例如，如果表达式引用了当前内部元组中的属性，
 *      我们需要知道当前的内部元组是什么，因此我们要查看表达式上下文。
 *
 *	There are two memory contexts associated with an ExprContext:
 *	与 ExprContext 关联的有两个内存上下文：
 *	* ecxt_per_query_memory is a query-lifespan context, typically the same
 *	  context the ExprContext node itself is allocated in.  This context
 *	  can be used for purposes such as storing function call cache info.
 *	  ecxt_per_query_memory 是查询生命周期的上下文，通常与分配 ExprContext
 *节点的上下文相同。 此上下文可用于存储函数调用缓存信息等目的。
 *	* ecxt_per_tuple_memory is a short-term context for expression results.
 *	  As the name suggests, it will typically be reset once per tuple,
 *	  before we begin to evaluate expressions for that tuple.  Each
 *	  ExprContext normally has its very own per-tuple memory context.
 *	  ecxt_per_tuple_memory 是表达式结果的短期上下文。
 *	  顾名思义，它通常在每个元组开始求值之前重置一次。
 *	  每个 ExprContext 通常都有自己专用的每元组内存上下文。
 *
 *	CurrentMemoryContext should be set to ecxt_per_tuple_memory before
 *	calling ExecEvalExpr() --- see ExecEvalExprSwitchContext().
 *	在调用 ExecEvalExpr() 之前，应将 CurrentMemoryContext 设置为
 *ecxt_per_tuple_memory
 *	--- 详见 ExecEvalExprSwitchContext()。
 * ----------------
 */
 ...
 *		that is, form new tuples by evaluation of targetlist
 *expressions. Nodes which need to do projections create one of these.
 *      这是执行投影所需的所有信息——即通过评估目标列表表达式来形成新的元组。
 *      需要执行投影的节点会创建其中之一。
 *
 *		The target tuple slot is kept in
 *ProjectionInfo->pi_state.resultslot. ExecProject() evaluates the tlist, forms
 *a tuple, and stores it in the given slot.  Note that the result will be a
 *"virtual" tuple unless ExecMaterializeSlot() is then called to force it to be
 *		converted to a physical tuple.  The slot must have a tupledesc
 *		that matches the output of the tlist!
 *      目标元组槽保存在 ProjectionInfo->pi_state.resultslot 中。
 *      ExecProject()
 *评估目标列表（tlist），形成一个元组，并将其存储在给定的槽中。
 *      请注意，除非随后调用 ExecMaterializeSlot() 强制将其转换为物理元组，
 *      否则结果将是一个“虚拟”元组。槽必须具有与 tlist 输出匹配的元组描述符！
 * ----------------
 */
 ...
 *
 *      此类用于存储有关“垃圾属性”的信息。垃圾属性是元组中的一种属性，它仅用于在执行器中存储中间信息，
 *      不属于最终输出的元组。例如，在执行 UPDATE
 *查询时，规划器会在目标列表中添加一个“垃圾”项， 以便返回给 ExecutePlan()
 *的元组包含一个额外属性：待更新元组的 ctid。 执行更新需要 ctid，但我们不希望
 *ctid 成为存储的新元组的一部分！
 *      因此，我们应用“垃圾过滤器”来移除垃圾属性并形成真实的输出元组。
 *      垃圾过滤器代码还提供了一些例程，用于从输入元组中提取垃圾属性的值。
 *
 *	  targetList:		the original target list (including junk
 *attributes). cleanTupType:		the tuple descriptor for the "clean"
 *tuple (with junk attributes removed). cleanMap:			A map
 *with the correspondence between the non-junk attribute numbers of the
 *"original" tuple and the attribute numbers of the "clean" tuple. resultSlot:
 *tuple slot used to hold cleaned tuple.
 * ----------------
 */
 ...
 * ON CONFLICT DO UPDATE 操作的执行器状态。
 */
 ...
 * 每当我们更新现有关系时，我们必须更新关系上的索引，也可能还要触发触发器。
 * ResultRelInfo 持有关于结果关系所需的所有信息，包括索引。
 *
 ...
 * 通常，ResultRelInfo 指向查询范围表中的表；此时 ri_RangeTableIndex 是 RT
 * 索引， 而 ri_RelationDesc 只是相关 es_relations[] 项的副本。
 * 然而，在某些情况下，我们会为不在范围表中的关系创建 ResultRelInfo，
 * 即针对分区表中的元组路由目标，以及在目标表以外的表中触发触发器时（参见
 * ExecGetTriggerResultRel）。 在这些情况下，ri_RangeTableIndex 为 0，而
 * ri_RelationDesc 是一个单独打开的 relcache 指针，需要单独关闭。
 */
...
typedef struct EState {
   ...
   * performed.
   * JIT 信息。es_jit_flags 指示是否应执行 JIT 以及使用哪些选项。
   * es_jit 在执行 JIT 时按需创建。
   *
   * es_jit_worker_instr is the combined, on demand allocated,
   * instrumentation from all workers. The leader's instrumentation is kept
   * separate, and is combined on demand by ExplainPrintJITSummary().
   * es_jit_worker_instr 是来自所有工作者的合并的、按需分配的检测信息。
   * leader 的检测信息是分开保存的，并由 ExplainPrintJITSummary() 按需合并。
   */
  int es_jit_flags;
   ...
   * 要在其上执行批量插入的外部表结果关系信息（ResultRelInfos）列表，
   * 以及所属的 ModifyTableStates，按相同顺序存储。
   */
 ...
 * 在执行 UPDATE/DELETE/MERGE/SELECT FOR [KEY] UPDATE/SHARE
 *时，查询中的每个非目标关系 （除了可以在运行时忽略的继承父级 RTE）都会有一个
 *ExecRowMark。 虚拟关系（如 FROM 中的子查询）将具有 relation == NULL 的
 *ExecRowMark。 有关大多数字段的详细信息，请参见 PlanRowMark。除了直接派生自
 *PlanRowMark 的字段外， 我们还存储一个 activity
 *标志（表示继承树的不活动子节点）、curCtid（由 WHERE CURRENT OF 代码使用） 和
 *ermExtra（可由提供关系的计划节点使用，例如 FDW 可以使用 ermExtra
 *存储外国表信息）。
 *
 ...
 * EState->es_rowmarks 是这些结构的数组，由 RT 索引编制索引，不相关的 RT 索引为
 *NULL。 如果没有 rowmark，则 es_rowmarks 本身为 NULL。
 */
 ...
 *     FOR [KEY] UPDATE/SHARE 子句的附加运行时表示
 *
 * Each LockRows and ModifyTable node keeps a list of the rowmarks it needs to
 * deal with.  In addition to a pointer to the related entry in es_rowmarks,
 * this struct carries the column number(s) of the resjunk columns associated
 * with the rowmark (see comments for PlanRowMark for more detail).
 * 每个 LockRows 和 ModifyTable 节点都持有一个其需要处理的 rowmark 列表。
 * 除了指向 es_rowmarks 中相关条目的指针外，此结构还包含与该 rowmark 关联的
 * resjunk 列的列号（更多详细信息请参见 PlanRowMark 的注释）。
 */
 ...
 * 注意：tab_hash_expr 用于对存储在表中的键数据类型进行哈希处理，
 * 而 tab_eq_func 是用于对这些类型进行相等性检查的非交叉类型 ExprState。
 * 通常这些是仅使用的 ExprState，但 FindTupleHashEntry()
 *支持使用交叉数据类型哈希搜索哈希表。 为此，调用者必须提供用于对左手边 (LHS)
 *数据类型进行哈希处理的 ExprState， 以及要使用的交叉类型相等性
 *ExprState。在进行此类搜索时，in_hash_expr 和 cur_eq_func
 * 被设置为指向调用者的哈希和相等性 ExprState。在执行 LookupTupleHashEntry()
 *时， 它们分别指向 tab_hash_expr 和 tab_eq_func。
 * ----------------------------------------------------------------
 */
 ...
 * 用于求值可能返回集合的表达式（如 FuncExpr 或 OpExpr）的状态。
 * 在某些情况下，如 ROWS FROM(...) 中的某些表达式可能不是 SRF，
 * 但它们仍然使用与 SRF 相同的机制；它将被视为返回单行的 SRF。
 * ----------------
 */
 ...
 * DomainConstraintState - CoerceToDomain 期间要检查的一个项目
 *
 ...
 * 注意：我们认为这是 ExprState 树的一部分，因此我们按照 xxxState 约定给它命名。
 * 但没有直接关联的计划树节点。
 */
 ...
 * 正在执行的查询具有一个 PlanState 树，该树与描述计划的 Plan 树平行。
 * ----------------------------------------------------------------
 */
 ...
 * 这是由 ExecProcNode 调用以从执行器节点返回下一个元组的方法。
 * 如果没有更多元组可用，它返回 NULL 或空的 TupleTableSlot。
 * ----------------
 */
 ...
 * 我们实际上永远不会实例化任何 PlanState 节点；它只是所有 PlanState
 *类型节点的通用抽象父类。
 * ----------------
 */
typedef struct PlanState {
  ...
  EState *state; /* at execution time, states of individual
                  ...
                  * top-level plan */
  /* 在执行期间，单个节点的各状态指向整个顶级计划的一个 EState */
   ...
   */
  /*
   * 所有计划类型的通用结构数据。这些到辅助状态树的链接与相关计划树中的链接平行
   * （subPlan 列表除外，它在计划树中不存在）。
   */
   ...
   */
  /*
   * 如果已知，则是 Scanslot 的描述符。这有点像黑客手段，但否则在这种情况下，
   * 如果不编码有关所有执行器节点的知识，表达式编译很难根据描述符进行优化。
   */
  TupleDesc scandesc;
   ...
   */
  /*
   * 为以该状态为父级的表达式上下文定义 inner、outer 和 scanslot 的槽类型。
   * 如果设置了 *opsset，则 *opsfixed 指示是否保证 *ops 是所使用的槽类型。
   * 这意味着在评估表达式时，相应 ExprContext.ecxt_*tuple
   * 中的每个槽都将指向该类型的槽。 如果 *opsfixed 为 false，但设置了
   * *ops，则指示最可能的槽类型。
   *
   * scan* 字段由 ExecInitScanTupleSlot()
   * 设置。如果未调用该函数，则节点可以自行初始化字段。
   *
   * 如果 outer/inneropsset 为 false，则在使用相应节点的 resultops* 字段对
   * ->righttree/lefttree 调用 ExecGetResultSlotOps() 时按需推断信息。
   *
   * 在使用 ExecInitResultSlot 时（无论是直接使用还是在通过
   * ExecAssignScanProjectionInfo() / ExecConditionalAssignProjectionInfo()
   * 创建槽时）， 将自动设置 result*
   * 字段。如果不需要投影，ExecConditionalAssignProjectionInfo()
   * 会将这些字段默认为扫描操作。
   */
 ...
 * 定义这些是为了避免“左”与“右”、“内”与“外”产生的混淆问题。惯例是
 * “左”计划是“外”计划，“右”计划是“内”计划，但这些宏使代码更具可读性。
 * ----------------
 */
 ...
 * 为了执行 EPQ，创建了一个单独的 EState（存储在 ->recheckestate 中），它与
 * 主查询的 EState（存储在 ->parentestate 中）共享某些资源，如范围表。
 * 需要重新检查的计划（子）树（在 ->plan 中）分别进行初始化（进入
 * ->recheckplanstate），
 * 但与主查询中的相应节点共享计划节点。该单独执行器树中的扫描节点被修改为仅返回各个表
 * 当前感兴趣的元组。这些元组要么由调用者提供（使用 EvalPlanQualSlot），
 * 要么使用 rowmark 机制找到（由 EPQ 机制本身在非锁定 rowmark 中寻找，锁定
 * rowmark 则由调用者寻找）。
 *
 * While the plan to be checked may be changed using EvalPlanQualSetPlan(),
 * all such plans need to share the same EState.
 * 虽然可以使用 EvalPlanQualSetPlan()
 * 更改要检查的计划，但所有此类计划都需要共享相同的 EState。
 */
 ...
 * regular ExprStates.
 * 注意：至少有一个“elems”将是 SetExprState；其余的是正则 ExprState。
 * ----------------
 */
 ...
 *		whichplan			which synchronous plan is being
 *executed (0 .. n-1) or a special negative value. See nodeAppend.c. prune_state
 *details required to allow partitions to be eliminated from the scan, or NULL
 *if not possible. valid_subplans		for runtime pruning, valid
 *synchronous appendplans indexes to scan.
 *      nplans            数组中有多少个计划
 *      whichplan         正在执行哪个同步计划（0 .. n-1）或特殊的负值。
 *                        参见 nodeAppend.c。
 *      prune_state       允许从扫描中排除分区所需的详细信息，如果不可能则为
 *NULL。 valid_subplans    用于运行时剪枝，要扫描的有效同步 appendplans 索引。
 * ----------------
 */
 ...
 *		initialized		true if we have fetched first tuple from
 *each subplan prune_state		details required to allow partitions to
 *be eliminated from the scan, or NULL if not possible. valid_subplans	for
 *runtime pruning, valid mergeplans indexes to scan.
 *      nplans          数组中有多少个计划
 *      nkeys           排序列的数量
 *      sortkeys        SortSupport 表示形式的排序列
 *      slots           每个子计划的当前输出元组
 *      heap            活动元组的堆
 *      initialized     如果我们已经从每个子计划中取出了第一个元组，则为 true
 *      prune_state     允许从扫描中排除分区所需的详细信息，如果不可能则为
 *NULL。 valid_subplans  用于运行时剪枝，要扫描的有效 mergeplans 索引。
 * ----------------
 */
 ...
 *
 *		recursing			T when we're done scanning the
 *non-recursive term intermediate_empty	T if intermediate_table is currently
 *empty working_table		working table (to be scanned by recursive term)
 *		intermediate_table	current recursive output (next
 *generation of WT)
 *      recursing            当我们完成扫描非递归项时为 T
 *      intermediate_empty   如果 intermediate_table 当前为空，则为 T
 *      working_table        工作表（由递归项扫描）
 *      intermediate_table   当前递归输出（WT 的下一代）
 * ----------------
 */
 ...
 *		that scan the output of an underlying plan node --- in that
 *case, only ScanTupleSlot is actually useful, and it refers to the tuple
 *		retrieved from the subplan.
 *      ScanState 为代表底层关系扫描的节点类型扩展了 PlanState。
 *      它也可以用于扫描底层计划节点输出的节点——在这种情况下，
 *      只有 ScanTupleSlot 是真正有用的，它指向从子计划中检索到的元组。
 *
 *		currentRelation    relation being scanned (NULL if none)
 *		currentScanDesc    current scan descriptor for scan (NULL if
 *none) ScanTupleSlot	   pointer to slot in tuple table holding scan tuple
 * ----------------
 */
 ...
 *		indexorderbyorig   execution state for indexorderbyorig
 ...
 *		SharedInfo		   parallel worker instrumentation (no
 *leader entry)
 *
 *		ReorderQueue	   tuples that need reordering due to re-check
 *		ReachedEnd		   have we fetched all tuples from index
 *already? OrderByValues	   values of ORDER BY exprs of last fetched
 *tuple OrderByNulls	   null flags for OrderByValues SortSupport
 *for reordering ORDER BY exprs OrderByTypByVals   is the datatype of order by
 *expression pass-by-value? OrderByTypLens	   typlens of the datatypes of
 *order by expressions PscanLen		   size of parallel index scan
 *descriptor
 *      indexqualorig       indexqualorig 表达式的执行状态
 *      indexorderbyorig    indexorderbyorig 表达式的执行状态
 *      ScanKeys            索引条件（qual）的 Skey 结构
 *      NumScanKeys         ScanKeys 的数量
 *      OrderByKeys         索引排序操作符的 Skey 结构
 *      NumOrderByKeys      OrderByKeys 的数量
 *      RuntimeKeys         关于必须在运行时评估的 Skey 的信息
 *      NumRuntimeKeys      RuntimeKeys 的数量
 *      RuntimeKeysReady    如果运行时 Skey 已计算，则为 true
 *      RuntimeContext      用于评估运行时 Skey 的表达式上下文
 *      RelationDesc        索引关系描述符
 *      ScanDesc            索引扫描描述符
 *      Instrument          局部索引扫描检测
 *      SharedInfo          并行工作者检测（无 leader 条目）
 *
 *      ReorderQueue        由于重新检查而需要重新排序的元组
 *      ReachedEnd          我们是否已经从索引中获取了所有元组？
 *      OrderByValues       最后获取元组的 ORDER BY 表达式的值
 *      OrderByNulls        OrderByValues 的空标志
 *      SortSupport         用于对 ORDER BY 表达式进行重新排序
 *      OrderByTypByVals    ORDER BY 表达式的数据类型是否为按值传递？
 *      OrderByTypLens      ORDER BY 表达式数据类型的 typlen
 ...
 *		Instrument		   local index scan instrumentation
 *		SharedInfo		   parallel worker instrumentation (no
 *leader entry) TableSlot		   slot for holding tuples fetched from
 ...
 *      Instrument          局部索引扫描检测
 *      SharedInfo          并行工作者检测（无 leader 条目）
 *      TableSlot           用于保存从表中获取的元组的槽
 *      VMBuffer            用于可见性地图测试的缓冲区（如果有）
 *      PscanLen            并行仅索引扫描描述符的大小
 *      NameCStringAttNums  需要填充到 NAMEDATALEN 的名称类型列的 attnum
 ...
 *evaluated at runtime NumRuntimeKeys	   number of RuntimeKeys ArrayKeys
 *info about Skeys that come from ScalarArrayOpExprs NumArrayKeys
 *number of ArrayKeys RuntimeKeysReady   true if runtime Skeys have been
 *computed RuntimeContext	   expr context for evaling runtime Skeys
 *		RelationDesc	   index relation descriptor
 *		ScanDesc		   index scan descriptor
 *		Instrument		   local index scan instrumentation
 ...
 *      RuntimeKeys         关于必须在运行时评估的 Skey 的信息
 *      NumRuntimeKeys      RuntimeKeys 的数量
 *      ArrayKeys           关于来自 ScalarArrayOpExprs 的 Skey 的信息
 ...
 *   BitmapHeapScanInstrumentation 信息
 *
 *		exact_pages		   total number of exact pages retrieved
 *		lossy_pages		   total number of lossy pages retrieved
 *      exact_pages        检索到的精确页面的总数
 *      lossy_pages        检索到的有损页面的总数
 * ----------------
 */
 ...
 *
 *		BM_INITIAL		TIDBitmap creation is not yet started,
 *so first worker to see this state will set the state to BM_INPROGRESS and that
 *process will be responsible for creating TIDBitmap. BM_INPROGRESS
 *TIDBitmap creation is in progress; workers need to sleep until it's finished.
 *		BM_FINISHED		TIDBitmap creation is done, so now all
 *workers can proceed to iterate over TIDBitmap.
 *      BM_INITIAL       TIDBitmap
 *创建尚未开始，因此第一个看到此状态的工作者将把 状态设置为
 *BM_INPROGRESS，该进程将负责创建 TIDBitmap。 BM_INPROGRESS    TIDBitmap
 *创建正在进行中；工作者在完成之前需要休眠。 BM_FINISHED      TIDBitmap
 *创建已完成，因此现在所有工作者都可以继续迭代 TIDBitmap。
 * ----------------
 */
typedef enum {
  BM_INITIAL,
 ...
 *current pages mutex					mutual exclusion for
 *state state					current state of the TIDBitmap
 *		cv						conditional wait
 *variable
 *      tbmiterator             用于扫描当前页面的迭代器
 *      mutex                   用于状态互斥
 ...
 *
 *		bitmapqualorig	   execution state for bitmapqualorig
 *expressions tbm				   bitmap obtained from child
 *index scan(s) stats			   execution statistics initialized
 *is node is ready to iterate pstate			   shared state for
 *parallel bitmap scan sinstrument		   statistics for parallel
 *workers recheck			   do current page's tuples need recheck
 *      bitmapqualorig      bitmapqualorig 表达式的执行状态
 *      tbm                 从子索引扫描获取的位图
 *      stats               执行统计信息
 *      initialized         节点是否准备好迭代
 *      pstate              并行位图扫描的共享状态
 *      sinstrument         并行工作者的统计信息
 *      recheck             当前页面的元组是否需要重新检查
 * ----------------
 */
 ...
 *		TidList		   evaluated item pointers (array of size
 *NumTids)
 *      tidexprs        TidExpr 结构列表（见 nodeTidscan.c）
 *      isCurrentOf     扫描具有 CurrentOfExpr 条件
 *      NumTids         此扫描中的 TID 数量
 *      TidPtr          当前获取的 TID 的索引
 *      TidList         已评估的项目指针（大小为 NumTids 的数组）
 * ----------------
 */
 ...
 *nodeTidrangescan.c) trss_mintid			the lowest TID in the
 *scan range trss_maxtid			the highest TID in the scan
 *range trss_inScan			is a scan currently in progress?
 *      trss_tidexprs       TidOpExpr 结构列表（见 nodeTidrangescan.c）
 *      trss_mintid         扫描范围中的最低 TID
 *      trss_maxtid         扫描范围中的最高 TID
 *      trss_inScan         当前是否正在进行扫描？
 * ----------------
 */
 ...
 *
 *		SubqueryScanState is used for scanning a sub-query in the range
 *table. ScanTupleSlot references the current output tuple of the sub-query.
 ...
 *      Function 节点用于扫描 FROM 中出现的函数的结果（通常是返回集合的函数）。
 *
 *		eflags				node's capability flags
 *		ordinality			is this scan WITH ORDINALITY?
 *		simple				true if we have 1 function and
 *no ordinality ordinal				current ordinal column value
 *		nfuncs				number of functions being
 *executed funcstates			per-function execution states (private
 *in nodeFunctionscan.c) argcontext			memory context to
 *evaluate function arguments in
 *      eflags              节点的评估标志
 *      ordinality          此扫描是否带有 WITH ORDINALITY？
 *      simple              如果我们只有 1 个函数且没有序数，则为 true
 *      ordinal             当前序数列的值
 *      nfuncs              正在执行的函数数量
 *      funcstates          每个函数的执行状态（在 nodeFunctionscan.c 中私有）
 ...
 *		exprlists			array of expression lists being
 *evaluated exprstatelists		array of expression state lists, for
 *SubPlans only array_len			size of above arrays curr_idx
 *current array index (0-based)
 *      rowcontext          每个表达式列表的上下文
 *      exprlists           正在评估的表达式列表数组
 *      exprstatelists      表达式状态列表数组，仅用于 SubPlan
 *      array_len           上述数组的大小
 *      curr_idx            当前数组索引（从 0 开始）
 *
 *	Note: ss.ps.ps_ExprContext is used to evaluate any qual or projection
 *	expressions attached to the node.  We create a second ExprContext,
 *	rowcontext, in which to build the executor expression state for each
 *	Values sublist.  Resetting this context lets us get rid of expression
 *	state for each row, avoiding major memory leakage over a long values
 *list. However, that doesn't work for sublists containing SubPlans, because a
 ...
 *  注意：ss.ps.ps_ExprContext 用于评估附加到节点的任何 qual 或投影表达式。
 *  我们创建第二个 ExprContext (rowcontext)，在其中为每个 Values
 *子列表构建执行器表达式状态。
 *  重置此上下文可以让我们摆脱每一行的表达式状态，从而避免在长 values
 *列表上发生重大内存泄漏。 但是，这对于包含 SubPlan 的子列表不起作用，因为
 *SubPlan 必须连接到外部计划树才能正常工作。 因此，仅对于那些包含 SubPlan
 *的子列表，我们在执行器启动时进行表达式状态构建， 并将这些指针存储在
 *exprstatelists[] 中。该数组中的 NULL 条目对应于如上所述处理的简单子表达式。
 * ----------------
 */
 ...
 * 多个 CteScan 节点可以从同一个 CTE 查询中读取数据。我们使用 tuplestore 来保存
 * 已从 CTE 查询中读取但尚未被所有读者消耗的行。
 * ----------------
 */
 ...
 *
 *		NamedTuplestoreScan nodes are used to scan a Tuplestore created
 *and named prior to execution of the query.  An example is a transition table
 *for an AFTER trigger.
 *      NamedTuplestoreScan 节点用于扫描在执行查询之前创建并命名的 Tuplestore。
 *      一个例子是 AFTER 触发器的转换表。
 *
 * Multiple NamedTuplestoreScan nodes can read out from the same Tuplestore.
 * 多个 NamedTuplestoreScan 节点可以从同一个 Tuplestore 中读取数据。
 * ----------------
 */
 ...
 *      WorkTableScan 节点用于扫描由 RecursiveUnion 节点创建的工作表。
 *      我们在执行器启动期间查找 RecursiveUnion 节点。
 * ----------------
 */
 ...
 *   CustomScanState 信息
 *
 *		CustomScan nodes are used to execute custom code within
 *executor.
 ...
 * 核心代码必须避免假设 CustomScanState 仅与此处声明的结构一样大；
 * 提供者被允许将其作为更大结构的第一个元素，并且通常需要这样做。
 * 该结构实际上是由与计划节点关联的 CreateCustomScanState 方法分配的。
 * 任何附加字段都可以在此处或 BeginCustomScan 方法中初始化。
 * ----------------
 */
...
typedef struct JoinState {
  ...
  /* 如果在找到一个内部匹配后应跳到下一个外层元组，则为 True */
  ExprState *joinqual; /* JOIN quals (in addition to ps.qual) */
                       /* JOIN 条件（除了 ps.qual 之外） */
} JoinState;
 ...
 *		MatchedOuter	   true if found a join match for current outer
 *tuple NullInnerTupleSlot prepared null tuple for left outer joins
 *      NeedNewOuter        如果下次调用需要新的外层元组，则为 true
 *      MatchedOuter        如果为当前外层元组找到了联接匹配，则为 true
 *      NullInnerTupleSlot  为左外联接准备的空元组
 * ----------------
 */
 ...
 *		JoinState		   current state of ExecMergeJoin state
 *machine SkipMarkRestore    true if we may skip Mark and Restore operations
 *		ExtraMarks		   true to issue extra Mark operations
 *on inner scan ConstFalseJoin	   true if we have a constant-false joinqual
 *		FillOuter		   true if should emit unjoined outer
 *tuples anyway FillInner		   true if should emit unjoined inner
 *tuples anyway MatchedOuter	   true if found a join match for current outer
 *tuple MatchedInner	   true if found a join match for current inner tuple
 *		OuterTupleSlot	   slot in tuple table for cur outer tuple
 *		InnerTupleSlot	   slot in tuple table for cur inner tuple
 *		MarkedTupleSlot    slot in tuple table for marked tuple
 *		NullOuterTupleSlot prepared null tuple for right outer joins
 *		NullInnerTupleSlot prepared null tuple for left outer joins
 *		OuterEContext	   workspace for computing outer tuple's join
 *values InnerEContext	   workspace for computing inner tuple's join values
 *      NumClauses          可合并联接的子句数量
 *      Clauses             每个可合并联接子句的信息
 *      JoinState           ExecMergeJoin 状态机的当前状态
 *      SkipMarkRestore     如果我们可能跳过 Mark 和 Restore 操作，则为 true
 *      ExtraMarks          为 true 则在内层扫描上发布额外的 Mark 操作
 *      ConstFalseJoin      如果具有恒定为假的 joinqual，则为 true
 *      FillOuter           即使未匹配也应发出未联接的外层元组，则为 true
 *      FillInner           即使未匹配也应发出未联接的内层元组，则为 true
 *      MatchedOuter        如果为当前外层元组找到了联接匹配，则为 true
 *      MatchedInner        如果为当前内层元组找到了联接匹配，则为 true
 *      OuterTupleSlot      用于当前外层元组的元组表槽
 *      InnerTupleSlot      用于当前内层元组的元组表槽
 *      MarkedTupleSlot     用于已标记元组的元组表槽
 *      NullOuterTupleSlot  为右外联接准备的空元组
 *      NullInnerTupleSlot  为左外联接准备的空元组
 *      OuterEContext       计算外层元组联接值的辅助空间
 *      InnerEContext       计算内层元组联接值的辅助空间
 * ----------------
 */
 ...
 *   HashJoinState 信息
 *
 *		hashclauses				original form of the
 *hashjoin condition hj_OuterHash			ExprState for hashing
 *outer keys hj_HashTable			hash table for the hashjoin
 *								(NULL if table
 *not built yet) hj_CurHashValue			hash value for current
 *outer tuple hj_CurBucketNo			regular bucket# for current
 *outer tuple hj_CurSkewBucketNo		skew bucket# for current outer
 *tuple hj_CurTuple				last inner tuple matched to
 *current outer tuple, or NULL if starting search (hj_CurXXX variables are
 *undefined if OuterTupleSlot is empty!) hj_OuterTupleSlot		tuple
 *slot for outer tuples hj_HashTupleSlot		tuple slot for inner
 *(hashed) tuples hj_NullOuterTupleSlot	prepared null tuple for
 *right/right-anti/full outer joins hj_NullInnerTupleSlot	prepared null
 *tuple for left/full outer joins hj_FirstOuterTupleSlot	first tuple
 *retrieved from outer plan hj_JoinState			current state of
 *ExecHashJoin state machine hj_MatchedOuter			true if found a
 *join match for current outer hj_OuterNotEmpty		true if outer relation
 *known not empty
 *      hashclauses             哈希联接条件的原始形式
 *      hj_OuterHash            用于对外部键进行哈希处理的 ExprState
 *      hj_HashTable            哈希联接的哈希表（如果尚未构建则为 NULL）
 *      hj_CurHashValue         当前外层元组的哈希值
 *      hj_CurBucketNo          当前外层元组的常规桶号
 *      hj_CurSkewBucketNo      当前外层元组的偏斜桶号
 *      hj_CurTuple 与当前外层元组匹配的最后一个内层元组，如果是开始搜索则为
 *NULL （如果 OuterTupleSlot 为空，则 hj_CurXXX 变量未定义！） hj_OuterTupleSlot
 *用于外层元组的元组槽 hj_HashTupleSlot        用于内层（哈希后）元组的元组槽
 *      hj_NullOuterTupleSlot   为右/右反/全外联接准备的空元组
 *      hj_NullInnerTupleSlot   为左/全外联接准备的空元组
 *      hj_FirstOuterTupleSlot  从外部计划检索到的第一个元组
 *      hj_JoinState            ExecHashJoin 状态机的当前状态
 *      hj_MatchedOuter         如果为当前外层找到了联接匹配，则为 true
 *      hj_OuterNotEmpty        如果已知外层关系不为空，则为 true
 * ----------------
 */
 ...
 *   MemoizeState 信息
 *
 *		memoize nodes are used to cache recent and commonly seen results
 *from a parameterized scan.
 *      memoize 节点用于缓存来自参数化扫描的最近且常用的结果。
 * ----------------
 */
 ...
 *   当执行多键排序时，输入数据集可能已经根据这些键的前缀进行了排序。
 *   我们称之为“预排序键（presorted keys）”。
 *   PresortedKeyData 表示关于此类键的信息。
 * ----------------
 */
 ...
 *  注意：ss.ps.ps_ExprContext 包含 ecxt_aggvalues 和 ecxt_aggnulls 数组，
 *  它们在评估 Agg 节点的输出元组期间保存当前输入组的计算聚合值。
 *  我们创建第二个 ExprContext
 *(tmpcontext)，用于评估输入表达式并运行聚合转换函数。
 * ---------------------
 */
 ...
 *      Unique 节点用于“在排序节点之上”以丢弃从排序阶段返回的重复元组。
 *      它所做的基本上就是将子计划中的当前元组与之前获取的元组（存储在其结果槽中）进行比较。
 *      如果两者在所有感兴趣的字段中都完全相同，那么我们只需从排序中获取另一个元组并再次尝试。
 * ----------------
 */
 ...
 *		The sorted mode is a bit like MergeJoin, the hashed mode like
 *Agg.
 *      SetOp 节点支持排序或哈希去重。
 *      排序模式有点像 MergeJoin，哈希模式有点像 Agg。
 * ----------------
 */
 ...
 *   LockRowsState 信息
 *
 *		LockRows nodes are used to enforce FOR [KEY] UPDATE/SHARE
 *locking.
 ...
 * 偏移量是要跳过的初始元组数（0 不执行任何操作）。
 * 计数是跳过偏移元组后要返回的元组数。
 * 如果未指定限制计数，则计数未定义且 noCount 为 true。
 * 当 lstate == LIMIT_INITIAL 时，偏移/计数/noCount 尚未设置。
 * ----------------
 */
typedef enum {
