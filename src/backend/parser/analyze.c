 ...
 *
 * 可选地，可以提供有关 $n 参数类型的信息。不允许引用未由 paramTypes[] 定义的 $n
 *索引。
 *
 ...
 *
 * 我们在这里做的唯一一件在 transformStmt() 中不做的操作是将 SELECT ... INTO
 *转换为 CREATE TABLE
 *AS。由于实用程序语句不允许在更大的语句中使用，因此这仅允许在解析树的
 ...
 * 语句类型，这都应该返回 true。当它返回 false
 *时，调用者可以假设不存在原始语句的 解析分析需要重新执行的情况。
 *
 ...
 *
 * 目前，由于重写器和规划器对 CMD_UTILITY 查询不执行任何操作，因此 false
 *结果意味着
 ...
 *		Returns true if rewriting or planning is non-trivial for this
 *Query.
 *
...
static Query *transformDeleteStmt(ParseState *pstate, DeleteStmt *stmt) {
  ...
  qual =
      transformWhereClause(pstate, stmt->whereClause, EXPR_KIND_WHERE, "WHERE");
...
static Query *transformInsertStmt(ParseState *pstate, InsertStmt *stmt) {
  ...
  if (selectStmt == NULL) {
     ...
     * see.
     *
     * 我们将 sub-pstate 设置为外部 pstate
     * 的子级，以便它可以查看从上方提供的任何 Param 定义。由于外部 pstate 的
     * rtable 和命名空间目前为空，因此公开 子 SELECT
     * 不应该能看到的名称没有副作用。
     */
     ...
     *
     * 重要的是，这要像独立的 SELECT 一样处理；否则 INSERT 内部 SELECT
     * 的行为可能 与独立的 SELECT 不同。（事实上，Postgres 直到 6.5
     * 版本都存在这类性质的 bug...）
     *
    ...
    nsitem = addRangeTableEntryForSubquery(
        pstate, selectQuery, makeAlias("*SELECT*", NIL), false, false);
     ...
     *
     * 技巧：SELECT
     *目标列表中的未知类型常量和参数将按原样复制，而不是作为子查询输出被引用。
     ...
     * 表达式列表的 VALUES RTE，并构建一个包含引用该 VALUES RTE 的 Var
     * 的目标列表。
     */
    List *exprsLists = NIL;
    List *coltypes = NIL;
    ...
    foreach (lc, selectStmt->valuesLists) {
       ...
       */
      sublist =
          transformExpressionList(pstate, sublist, EXPR_KIND_VALUES, true);
     ...
     */
    nsitem =
        addRangeTableEntryForValues(pstate, exprsLists, coltypes, coltypmods,
                                    colcollations, NULL, lateral, true);
    addNSItemToQuery(pstate, nsitem, true, false, false);
  ...
  forthree(lc, exprList, icols, icolumns, attnos, attrnos) {
    ...
    perminfo->insertedCols = bms_add_member(
        perminfo->insertedCols, attr_num - FirstLowInvalidHeapAttributeNumber);
  }
 ...
 */
List *transformInsertRow(ParseState *pstate, List *exprlist, List *stmtcols,
  ...
  if (list_length(exprlist) > list_length(icolumns))
    ereport(
        ...
        (errcode(ERRCODE_SYNTAX_ERROR),
         ...
         parser_errposition(
             pstate, exprLocation(list_nth(exprlist, list_length(icolumns))))));
        ...
        (errcode(ERRCODE_SYNTAX_ERROR),
         ...
         ((list_length(exprlist) == 1 &&
           count_rowexpr_columns(pstate, linitial(exprlist)) ==
              ...
              ? errhint("The insertion source is a row expression containing "
                        "the same number of columns expected by the INSERT. "
                        "Did you accidentally use extra parentheses?")
              : 0),
         parser_errposition(
             pstate, exprLocation(list_nth(icolumns, list_length(exprlist))))));
  }
  ...
  forthree(lc, exprlist, icols, icolumns, attnos, attrnos) {
    ...
    expr =
        transformAssignedExpr(pstate, expr, EXPR_KIND_INSERT_TARGET, col->name,
                              attno, col->indirection, col->location);
...
transformOnConflictClause(ParseState *pstate,
  ...
  if (onConflictClause->action == ONCONFLICT_UPDATE) {
    ...
    exclNSItem =
        addRangeTableEntryForRelation(pstate, targetrel, RowExclusiveLock,
                                      makeAlias("excluded", NIL), false, false);
  ...
  if (onConflictClause->action == ONCONFLICT_UPDATE) {
    ...
    onConflictWhere = transformWhereClause(
        pstate, onConflictClause->whereClause, EXPR_KIND_WHERE, "WHERE");
...
List *BuildOnConflictExcludedTargetlist(Relation targetrel,
   ...
   */
  var = makeVar(exclRelIndex, InvalidAttrNumber, targetrel->rd_rel->reltype, -1,
                InvalidOid, 0);
...
static Query *transformSelectStmt(ParseState *pstate, SelectStmt *stmt) {
  ...
  if (stmt->intoClause)
    ereport(ERROR, (errcode(ERRCODE_SYNTAX_ERROR),
                    ...
                    parser_errposition(
                        pstate, exprLocation((Node *)stmt->intoClause))));
  ...
  /* 转换目标列表 */
  qry->targetList =
      transformTargetList(pstate, stmt->targetList, EXPR_KIND_SELECT_TARGET);
   ...
   */
  qry->sortClause =
      transformSortClause(pstate, stmt->sortClause, &qry->targetList,
                          EXPR_KIND_ORDER_BY, false /* allow SQL92 rules */);

  qry->groupClause = transformGroupClause(
      pstate, stmt->groupClause, &qry->groupingSets, &qry->targetList,
      qry->sortClause, EXPR_KIND_GROUP_BY, false /* allow SQL92 rules */);
  ...
  if (stmt->distinctClause == NIL) {
    ...
    qry->distinctClause = transformDistinctOnClause(
        pstate, stmt->distinctClause, &qry->targetList, qry->sortClause);
    qry->hasDistinctOn = true;
  }

  /* transform LIMIT */
  qry->limitOffset = transformLimitClause(
      pstate, stmt->limitOffset, EXPR_KIND_OFFSET, "OFFSET", stmt->limitOption);
  qry->limitCount = transformLimitClause(
      pstate, stmt->limitCount, EXPR_KIND_LIMIT, "LIMIT", stmt->limitOption);
 ...
 *
 * 我们构建一个包含 VALUES RTE 的 Query，就像写成了 SELECT * FROM (VALUES ...)
 *AS "*VALUES*" 一样
 */
static Query *transformValuesClause(ParseState *pstate, SelectStmt *stmt) {
  Query *qry = makeNode(Query);
  ...
  foreach (lc, stmt->valuesLists) {
     ...
     *
     * 执行基础表达式转换（与 ROW() 表达式相同，但在此处我们不允许使用
     * SetToDefault）
     */
   ...
   * 不处理范围表条目， 并且 (2) 我们需要为 VALUES RTE
   * 标记列排序规则，以便在外部查询中使用。
   * 我们在这里不认为隐式排序规则的冲突是错误；相反，该列将仅显示 InvalidOid
   * 作为其排序规则， 如果这导致解析排序规则失败，您稍后将收到失败消息。
   *
   ...
   * 生成 VALUES RTE
   */
  nsitem = addRangeTableEntryForValues(pstate, exprsLists, coltypes, coltypmods,
                                       colcollations, NULL, lateral, true);
    ...
    ereport(
        ...
        (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
         ...
         errmsg(
             ...
             LCS_asString(
                 ((LockingClause *)linitial(stmt->lockingClause))->strength))));
...
static Query *transformSetOperationStmt(ParseState *pstate, SelectStmt *stmt) {
  ...
  if (leftmostSelect->intoClause)
    ereport(ERROR,
            (errcode(ERRCODE_SYNTAX_ERROR),
             errmsg("SELECT ... INTO is not allowed here"),
             parser_errposition(
                 pstate, exprLocation((Node *)leftmostSelect->intoClause))));
   ...
   * right back here!
   *
   * 我们需要在此处提取 ORDER BY 和其他顶层子句，而不让
   * transformSetOperationTree() 看到它们 —— 否则它只会直接递归回到这里！
   */
    ...
    ereport(
        ...
        (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
         ...
         errmsg("%s is not allowed with UNION/INTERSECT/EXCEPT",
                LCS_asString(
                    ((LockingClause *)linitial(lockingClause))->strength))));
   ...
   * 为外部查询生成虚拟目标列表。还生成虚拟变量及其名称的列表，以便在解析 ORDER
   * BY 时使用。
   *
   ...
   *
   * 注意：我们使用 leftmostRTI 作为虚拟变量的 varno。它们具有哪个 RT
   * 索引并不重要， 只要它们具有与实际 RT
   * 条目相对应的索引即可；否则，当规则重写捣碎树时，可能会发生有趣的事情。
   */
  qry->targetList = NIL;
  targetvars = NIL;
  targetnames = NIL;
  sortnscolumns = (ParseNamespaceColumn *)palloc0(
      list_length(sostmt->colTypes) * sizeof(ParseNamespaceColumn));
  sortcolindex = 0;

  forfour(lct, sostmt->colTypes, lcm, sostmt->colTypmods, lcc,
          sostmt->colCollations, left_tlist, leftmostQuery->targetList) {
   ...
   * "foo not found".
   *
   * 注意：我们目前还没有对这种情况做任何有用的处理，但至少 "ORDER BY
   * upper(foo)" 将引出正确的错误消息，而不是 "foo not found"。
   */
  ...
  qry->sortClause =
      transformSortClause(pstate, sortClause, &qry->targetList,
                          EXPR_KIND_ORDER_BY, false /* allow SQL92 rules */);
  ...
  if (tllen != list_length(qry->targetList))
    ereport(ERROR, (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
                    ...
                    errhint("Add the expression/function to every SELECT, or "
                            "move the UNION into a FROM clause."),
                    parser_errposition(pstate, exprLocation(list_nth(
                                                   qry->targetList, tllen)))));

  qry->limitOffset = transformLimitClause(pstate, limitOffset, EXPR_KIND_OFFSET,
                                          "OFFSET", stmt->limitOption);
  qry->limitCount = transformLimitClause(pstate, limitCount, EXPR_KIND_LIMIT,
                                         "LIMIT", stmt->limitOption);
  qry->limitOption = stmt->limitOption;
...
SortGroupClause *makeSortGroupClauseForSetOp(Oid rescoltype,
  ...
  get_sort_group_operators(rescoltype, false, true, false, &sortop, &eqop, NULL,
                           &hashable);
 ...
 *
 * 除了返回转换后的节点外，如果 targetlist 不为 NULL，则返回其非 resjunk
 *TargetEntry
...
static Node *transformSetOperationTree(ParseState *pstate, SelectStmt *stmt,
  ...
  if (isLeaf) {
     ...
     *
     * 这与正常的 SELECT 转换工作方式相同，除了我们阻止将未知类型的输出解析为
     * TEXT。 这不会改变子查询的语义，因为如果列类型在语义上很重要，
      ...
      if (contain_vars_of_level((Node *)selectQuery, 1))
        ereport(ERROR,
                (errcode(ERRCODE_INVALID_COLUMN_REFERENCE),
                 errmsg("UNION/INTERSECT/EXCEPT member statement cannot refer "
                        "to other relations of same query level"),
                 parser_errposition(
                     pstate, locate_var_of_level((Node *)selectQuery, 1))));
    }
    ...
    nsitem = addRangeTableEntryForSubquery(
        pstate, selectQuery, makeAlias(selectName, NIL), false, false);
    ...
    const char *context;
    bool recursive =
        (pstate->p_parent_cte && pstate->p_parent_cte->cterecursive);
    /* 是否为递归查询 */
     ...
     */
    op->larg =
        transformSetOperationTree(pstate, stmt->larg, false, &ltargetlist);
     ...
     */
    op->rarg =
        transformSetOperationTree(pstate, stmt->rarg, false, &rtargetlist);
    ...
    forboth(ltl, ltargetlist, rtl, rtargetlist) {
       ...
       *
       * 如果子表达式是一个 UNKNOWN 类型的 Const 或
       *Param，我们希望将其替换为强制转换后的表达式。
       * 这仅在子查询是一个叶子集合操作节点时发生。替换表达式是安全的，因为如果子查询的语义
       * 依赖于此输出列的类型，它肯定已经将 UNKNOWN
       *强制转换为其他类型了。我们希望这样做， 因为 (a) 我们希望验证 Const
       *对于目标类型是否有效，或者解析 UNKNOWN Param 的实际类型，以及 (b)
       *我们希望避免子查询的输出类型与解析后的目标类型之间出现
       ...
       *（知道 coerce_to_common_type
       *会失败）。规划器有时能够在必须强制转换类型之前 将 UNKNOWN Var
       *折叠为常量，因此现在失败只会破坏可能有效的情况。
       */
      ...
      rescoltypmod = select_common_typmod(
          pstate, list_make2(lcolnode, rcolnode), rescoltype);
      ...
      if (targetlist) {
        ...
        restle =
            makeTargetEntry((Expr *)rescolnode, 0, /* no need to set resno */
                            NULL, false);
...
List *transformUpdateTargetList(ParseState *pstate, List *origTlist) {
  ...
  if (pstate->p_next_resno <=
      RelationGetNumberOfAttributes(pstate->p_target_relation))
    pstate->p_next_resno =
        RelationGetNumberOfAttributes(pstate->p_target_relation) + 1;
      ...
      ereport(ERROR,
              (errcode(ERRCODE_UNDEFINED_COLUMN),
               ...
               (origTarget->indirection != NIL &&
                strcmp(origTarget->name,
                       pstate->p_target_nsitem->p_names->aliasname) == 0)
                   ? errhint("SET target columns cannot be qualified with the "
                             "relation name.")
                   : 0,
 ...
 */
static void addNSItemForReturning(ParseState *pstate, const char *aliasname,
  ...
  nscolumns =
      (ParseNamespaceColumn *)palloc(numattrs * sizeof(ParseNamespaceColumn));
 ...
 */
void transformReturningClause(ParseState *pstate, Query *qry,
  ...
  foreach_node(ReturningOption, option, returningClause->options) {
    ...
    if (refnameNamespaceItem(pstate, NULL, option->value, -1, NULL) != NULL)
      ereport(
          ERROR, errcode(ERRCODE_DUPLICATE_ALIAS),
          errmsg("table name \"%s\" specified more than once", option->value),
          parser_errposition(pstate, option->location));
  ...
  qry->returningList =
      transformTargetList(pstate, returningClause->exprs, exprKind);
  ...
  if (qry->returningList == NIL)
    ereport(ERROR, (errcode(ERRCODE_SYNTAX_ERROR),
                    ...
                    parser_errposition(pstate, exprLocation(linitial(
...
static Query *transformPLAssignStmt(ParseState *pstate, PLAssignStmt *stmt) {
   ...
   */
  /*
   * 其余部分大部分与 transformSelectStmt 匹配，不同之处在于我们不需要考虑 WITH
   * 或 INTO， 并且我们以自己的方式构建目标列表。
   */
  qry->commandType = CMD_SELECT;
  pstate->p_is_insert = false;
  ...
  tlist =
      transformTargetList(pstate, sstmt->targetList, EXPR_KIND_SELECT_TARGET);
  ...
  if (list_length(tlist) != 1)
    ereport(ERROR, (errcode(ERRCODE_SYNTAX_ERROR),
                    errmsg_plural("assignment source returned %d column",
                                  ...
                                  list_length(tlist), list_length(tlist))));
  ...
  if (indirection) {
    tle->expr = (Expr *)transformAssignmentIndirection(
        pstate, target, stmt->name, false, targettype, targettypmod,
        targetcollation, indirection, list_head(indirection), (Node *)tle->expr,
        COERCION_PLPGSQL, exprLocation(target));
    ...
    tle->expr = (Expr *)coerce_to_target_type(
        pstate, orig_expr, type_id, targettype, targettypmod, COERCION_PLPGSQL,
      ...
      ereport(ERROR,
              (errcode(ERRCODE_DATATYPE_MISMATCH),
               errmsg("variable \"%s\" is of type %s"
                      ...
                      stmt->name, format_type_be(targettype),
  ...
  /* transform WHERE */
  qual = transformWhereClause(pstate, sstmt->whereClause, EXPR_KIND_WHERE,
                              "WHERE");
   ...
   */
  qry->sortClause =
      transformSortClause(pstate, sstmt->sortClause, &qry->targetList,
                          EXPR_KIND_ORDER_BY, false /* allow SQL92 rules */);

  qry->groupClause = transformGroupClause(
      pstate, sstmt->groupClause, &qry->groupingSets, &qry->targetList,
      qry->sortClause, EXPR_KIND_GROUP_BY, false /* allow SQL92 rules */);
  ...
  if (sstmt->distinctClause == NIL) {
    ...
    /* We had SELECT DISTINCT ON */
    qry->distinctClause = transformDistinctOnClause(
        pstate, sstmt->distinctClause, &qry->targetList, qry->sortClause);
    qry->hasDistinctOn = true;
  }

  /* transform LIMIT */
  qry->limitOffset =
      transformLimitClause(pstate, sstmt->limitOffset, EXPR_KIND_OFFSET,
                           "OFFSET", sstmt->limitOption);
  qry->limitCount = transformLimitClause(
      pstate, sstmt->limitCount, EXPR_KIND_LIMIT, "LIMIT", sstmt->limitOption);
 ...
 *
 * DECLARE CURSOR 与其他实用程序语句相似，我们将它作为一个 CMD_UTILITY Query
 *节点发出； 但是，我们必须首先转换包含的查询。我们过去常将其推迟到执行时，
...
static Query *transformDeclareCursorStmt(ParseState *pstate,
  ...
  if ((stmt->options & CURSOR_OPT_ASENSITIVE) &&
    ...
    ereport(ERROR, (errcode(ERRCODE_INVALID_CURSOR_DEFINITION),
                    /* translator: %s is a SQL keyword */
                    errmsg("cannot specify both %s and %s", "ASENSITIVE",
                           "INSENSITIVE")));
    ...
    ereport(
        ...
        (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
         ...
         errmsg("DECLARE CURSOR WITH HOLD ... %s is not supported",
                LCS_asString(
                    ((RowMarkClause *)linitial(query->rowMarks))->strength)),
...
static Query *transformCreateTableAsStmt(ParseState *pstate,
  ...
  if (stmt->objtype == OBJECT_MATVIEW) {
     ...
     */
    /*
     * 禁止在用于创建物化视图的查询中使用修改数据的
     * CTE。如果物化视图被刷新或增量维护， 尚不清楚用户希望发生什么。
     */
    if (query->hasModifyingCTE)
      ereport(ERROR, (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
                      errmsg("materialized views must not use data-modifying "
                             "statements in WITH")));
     ...
     */
    /*
     * 物化视图要么需要保存参数以用于维护/加载数据，要么完全禁止它们。
     * 后者似乎更安全也更理智。
     */
    if (query_contains_extern_params(query))
      ereport(ERROR, (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
                      errmsg("materialized views may not be defined using "
                             "bound parameters")));
     ...
     */
    /*
     * 目前我们禁止非日志记录（unlogged）的物化视图，因为崩溃后它们直接变空似乎不是个好主意。
     *（如果我们能将它们标记为未填充，那会更好，但这需要目录更改，崩溃恢复目前无法处理。）
     */
     ...
     */
    /*
     * 在运行时，我们需要一个解析后但未重写的 Query 副本，以便创建视图的 ON
     * SELECT 规则。 我们将其存储在 IntoClause 中，因为那是 intorel_startup()
     * 可以方便地从中获取它的地方。
     */
...
static Query *transformCallStmt(ParseState *pstate, CallStmt *stmt) {
   ...
   */
  /*
   * 首先，对过程调用及其参数执行标准解析分析，使我们能够识别被调用的过程。
   */
  targs = NIL;
  ...
  node = ParseFuncOrColumn(pstate, stmt->funccall->funcname, targs,
                           pstate->p_last_srf, stmt->funccall, true,
   ...
   */
  /*
   * 扩展参数列表以处理命名参数表示法和默认参数。对于普通的 FuncExprs，
   * 这将在规划期间完成，但 CallStmt
   * 不经过规划，且似乎没有理由不在此处执行此操作。
   */
  fexpr->args = expand_function_arguments(fexpr->args, true,
                                          fexpr->funcresulttype, proctup);

  /* Fetch proargmodes; if it's null, there are no output args */
  /* 获取 proargmodes；如果它为空，则表示没有输出参数 */
  proargmodes =
  ...
  if (!isNull) {
     ...
     */
    /*
     * 将列表拆分为 fexpr->args 中的输入参数和 stmt->outargs 中的输出参数。
     * INOUT 参数出现在两个列表中。
     */
    ArrayType *arr;
    int numargs;
    ...
    if (ARR_NDIM(arr) != 1 || ARR_DIMS(arr)[0] != numargs || ARR_HASNULL(arr) ||
        ARR_ELEMTYPE(arr) != CHAROID)
      elog(ERROR,
           "proargmodes is not a 1-D char array of length %d or it contains "
           "nulls",
           numargs);
 ...
 */
/*
 * 生成 LockClauseStrength 值的字符串表示形式。
 ...
 */
/*
 * 检查 FOR [KEY] UPDATE/SHARE 不支持的功能。
 *
 * 已导出，以便规划器在重写、查询上拉等之后再次检查
 */
  ...
  if (qry->hasTargetSRFs)
    ereport(ERROR,
            (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
             /*------
               translator: %s is a SQL row locking clause such as FOR UPDATE */
             errmsg("%s is not allowed with set-returning functions in the "
                    "target list",
                    LCS_asString(strength))));
}
...
static void transformLockingClause(ParseState *pstate, Query *qry,
  ...
  if (lockedRels == NIL) {
     ...
     */
    /*
     * 锁定查询及其子查询中使用的所有常规表。我们检查 inFromCl 以排除自动添加的
     * RTE， 特别是规则中的
     * NEW/OLD。这有点滥用了一个大多已过时的标志，但它很方便。
     * 我们不能依赖已很大程度上取代 inFromCl 的命名空间机制，因为例如即使
     * 基本关系 RTE 被上层连接遮蔽，我们仍需要锁定它们。
     */
    i = 0;
    foreach (rt, qry->rtable) {
      ...
      case RTE_SUBQUERY:
         ...
         */
        /*
         * 子查询的 FOR UPDATE/SHARE 也会传播到子查询的所有关系中。
         * 我们稍后可以执行此操作（根据子查询 RTE 的标记），但在每个查询层级
         * 拥有关于哪些关系需要使用 RowShareLock 打开的局部知识是很方便的。
         */
     ...
     */
    /*
     * 仅锁定命名的表。如上所述，无论别名可见性规则如何，我们都允许锁定任何
     * 基本关系，因此我们需要检查 inFromCl 以排除 OLD/NEW。
     */
    foreach (l, lockedRels) {
      ...
      foreach (rt, qry->rtable) {
         ...
         */
        /*
         * 没有别名的连接 RTE 作为关系名称不可见，需要跳过（否则它可能会隐藏
         * 具有相同名称的基本关系），除非它具有可见的 USING 别名。
         *
         * 没有别名的子查询和 values RTE
         * 永远不会作为关系名称可见，必须始终跳过。
         */
        if (rte->alias == NULL) {
        ...
        if (strcmp(rtename, thisrel->relname) == 0) {
          ...
          case RTE_JOIN:
            ereport(ERROR, (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
                            /*------
                              translator: %s is a SQL row locking clause such as
                              FOR UPDATE */
        ...
        ereport(
            ...
            (errcode(ERRCODE_UNDEFINED_TABLE),
             ...
             errmsg("relation \"%s\" in %s clause not found in FROM clause",
                    thisrel->relname, LCS_asString(lc->strength)),
             parser_errposition(pstate, thisrel->location)));
    }
  }
}
...
void applyLockingClause(Query *qry, Index rtindex, LockClauseStrength strength,
  ...
  /* 如果它是一个显式子句，确保 hasForUpdate 被设置 */
  if (!pushedDown)
    qry->hasForUpdate = true;

  /* Check for pre-existing entry for same rtindex */
  /* 检查是否存在相同 rtindex 的现有条目 */
  if ((rc = get_parse_rowmark(qry, rtindex)) != NULL) {
     ...
     */
    /*
     * 如果同一个 RTE 指定了多个锁定强度，请使用最强的一个。（这是合理的，因为您
     * 不能同时获取共享锁和排他锁；它最终无论如何都会变成排他锁。）
     *
     * 同样地，如果同一个 RTE 指定了多个锁定等待策略，则认为 NOWAIT 胜过
     * SKIP LOCKED，而后者又胜过等待锁（默认值）。这有点更具争议性，
     * 但抛出错误似乎没有帮助。（例如，考虑从内部包含普通 FOR UPDATE 规范的
     * 视图中执行 SELECT FOR UPDATE NOWAIT。）让 NOWAIT 胜过 SKIP LOCKED
     * 是合理的，因为前者在遇到锁定元组时会抛出错误，这在某些情况下可能是不希望的，
     * 但这似乎比默默返回不一致的结果要好。
     *
     * 当然，如果任何子句是显式的，pushedDown 就会变为 false。
     */
 ...
 *
 * raw_expression_tree_walker() 的覆盖率测试。
 *
 * 启用后，我们对提交给解析分析的每个 DML 语句运行
 * raw_expression_tree_walker()。 如果没有此规定，该函数仅在涉及 CTE
 * 的有限情况下应用， 且我们不希望必须测试 CTE 内部和外部的所有内容。
 */
static bool test_raw_expression_coverage(Node *node, void *context) {
  if (node == NULL)
    return false;
  return raw_expression_tree_walker(node, test_raw_expression_coverage,
                                    context);
}
#endif /* DEBUG_NODE_TESTS_ENABLED */
