 ...
 *
 * initdb --- 初始化 PostgreSQL 安装
 *
 * initdb creates (initializes) a PostgreSQL database cluster (site,
 * instance, installation, whatever).  A database cluster is a
 * collection of PostgreSQL databases all managed by the same server.
 *
 * initdb 创建（初始化）一个 PostgreSQL 数据库集群（站点、实例、安装等）。
 * 数据库集群是由同一个服务器管理的一组 PostgreSQL 数据库。
 *
 ...
 * database "postgres".
 *
 * 为了创建数据库集群，我们需要创建包含其所有数据的目录，创建存储全局
 * 表的文件，创建一些其他的控制文件，并创建三个数据库：模板数据库 
 * "template0" 和 "template1"，以及一个默认的用户数据库 "postgres"。
 *
 ...
 *
 * 模板数据库是普通的 PostgreSQL 数据库。template0 在 initdb 之后理论上不
 * 应该再改变，而 template1 可以用来添加站点本地的标准数据。两者都可以
 * 被复制来产生新的数据库。
 *
 ...
 *
 * 由于很大一部分历史原因，template1 数据库是由基本的引导（bootstrap）
 * 过程构建的。在它完成后，template0 和默认数据库 postgres 仅仅是通过
 * 复制 template1 制作的。
 *
 ...
 *
 * 这是以前用于设置 PostgreSQL 集群位置的 shell 脚本的 C 实现，且应与
 * 其高度兼容。
 * C 翻译作者：Andrew Dunstan	   mailto:andrew@dunslane.net
 *
 * This code is released under the terms of the PostgreSQL License.
 *
 * 此代码根据 PostgreSQL 许可证的条款进行发布。
 *
 * Portions Copyright (c) 1996-2025, PostgreSQL Global Development Group
 * Portions Copyright (c) 1994, Regents of the University of California
 *
 * 部分版权所有 (c) 1996-2025, PostgreSQL 全球开发组
 * 部分版权所有 (c) 1994, 加州大学校董会
 *
...
static const char *const subdirs[] = {"global",
...
static char **replace_guc_value(char **lines, const char *guc_name,
                                const char *guc_value, bool mark_as_comment);
 ...
 *
 * 保存给定类别的当前全局区域设置（locale）名称的副本。
 * 返回的值必须传递给 restore_global_locale()。
 *
 ...
 *
 * 由于来自环境的名称尚未经过非 ASCII 字符验证，我们在 Windows 上使用 
 * setlocale() 的 wchar_t 变型。否则，在保存-恢复的往返过程中，
 * 当我们中途切换到另一个区域设置后，普通的 setlocale() 在恢复时 
 * 可能会用不同的编码来解释该名称本身。（此问题仅存在于 initdb 中， 
 * 在类似的后端代码中不存在，因为那里的全局区域设置名称此前应已
 * 验证为仅限 ASCII。）
 */
 ...
 *
 * 恢复由 save_global_locale() 返回的全局区域设置。
 */
 ...
 *
 * 转义单引号和反斜杠，适用于插入配置文件或 SQL E'' 字符串。
 */
static char *escape_quotes(const char *src) {
 ...
 *
 * 转义要插入到 BKI 数据中的字段值。
 * 通过 escape_quotes 运行该值（该过程将由后端的 
 * DeescapeQuotedString() 函数反向处理），然后将该值用单引号 
 * 包裹，即便这并非严格必要。
 */
static char *escape_quotes_bki(const char *src) {
  char *result;
 ...
 *
 * 修改行数组，将可能被注释掉的参数 guc_name 的赋值替换为 guc_value 的
 * 有效赋值。该值将被适当引用。
 *
...
static char **replace_guc_value(char **lines, const char *guc_name,
  ...
  for (i = 0; lines[i]; i++) {
     ...
     *
     * 寻找一个给 guc_name 赋值的行。通常它会被 '#' 前导，但如果 
     * -c 开关覆盖了之前的赋值，情况可能就不是这样。我们也允许 
     * 前导空格，尽管通常不会有。
     */
    where = lines[i];
    ...
    if (where) {
       ...
       *
       * 我们尝试保留原始缩进，这很乏味。
       * oldindent 和 newindent 以去制表符（de-tab-ified）的列来衡量。
       */
      const char *ptr;
      int oldindent = 0;
      int newindent;
 ...
 *
 * 决定是否应对替换后的 GUC 值加引号。我们在这里并不太苛求，
 * 但我们希望避免对简单的标识符和带单位的数字加引号，这些
 * 都是常见情况。
 */
static bool guc_value_requires_quotes(const char *guc_value) {
  /* Don't use <ctype.h> macros here, they might accept too much */
  /* 在这里不要使用 <ctype.h> 宏，它们可能会接受太多东西 */
 ...
 *
 * 结果是一个 malloc 分配的数组，包含单独 malloc 分配的字符串。
 */
static char **readfile(const char *path) {
  char **result;
  FILE *infile;
 ...
 *
 * "lines" 必须是一个由单独 malloc 分配的字符串组成的 malloc 分配的数组。
 * 所有这些数据在这里都会被释放。
 *
 * This is only used to write text files.  Use fopen "w" not PG_BINARY_W
 * so that the resulting configuration files are nicely editable on Windows.
 *
 * 这仅用于编写文本文件。使用 fopen "w" 而非 PG_BINARY_W，
 * 以便生成的配置文件在 Windows 上易于编辑。
 */
static void writefile(char *path, char **lines) {
  FILE *out_file;
  char **line;
 ...
 *
 * 支持确定最佳默认文本搜索配置。
 * 我们根据 LC_CTYPE 的第一部分（即语言名称）来确定这一点。
 */
...
static const struct tsearch_config_match tsearch_config_languages[] = {
    ...
    {"russian", "Russian"}, {"serbian", "sr"},
    {"serbian", "Serbian"}, {"spanish", "es"},
    {"spanish", "Spanish"}, {"swedish", "sv"},
    {"swedish", "Swedish"}, {"tamil", "ta"},
    {"tamil", "Tamil"},     {"turkish", "tr"},
    {"turkish", "Turkish"}, {"yiddish", "yi"},
    {"yiddish", "Yiddish"}, {NULL, NULL} /* end marker */
    /* 结束标记 */
};
 ...
 * memory instead.
 *
 * 确定在此平台上应使用哪种动态共享内存实现。POSIX 
 * 共享内存更可取，因为大部支持两者的系统其默认分配限制远高于 
 * System V，但一个平台拥有 shm_open 并不保证该调用在尝试时
 * 肯定会成功。因此，我们尝试重现 postmaster 在 dsm_impl.c 
 * 中分配 POSIX 段时的行为；如果它不起作用，我们就假设它
 * 对 postmaster 也不起作用，并将集群配置为使用 System V 
 * 共享内存。
 *
 * We avoid choosing Solaris's implementation of shm_open() by default.  It
 * can sleep and fail spuriously under contention.
 *
 * 我们默认避免选择 Solaris 版本的 shm_open()。
 * 它在存在竞争时可能会进入睡眠并产生虚假错误。
 */
...
static void test_config_settings(void) {
   ...
   *
   * 此宏定义了对于给定的 max_connections 值，我们期望的最少 
   * shared_buffers。数组显示了待尝试的设置。
   */
#define MIN_BUFS_FOR_CONNS(nconns) ((nconns) * 10)
   ...
   *
   * 此宏定义了对于给定的 max_connections 值，我们期望的 
   * autovacuum_worker_slots 默认值。请注意，它经过精心设计，
   * 旨在为 trial_conns 中的关联值提供特定值。我们希望它在 
   * trial_conns[] 中的最大值 (100) 时返回 autovacuum_worker_slots 
   * 的初始默认值 (16)，同时对于 trial_conns[] 中的最小值，
   * 绝不能返回小于 autovacuum_max_workers 的默认值 (3)。
   */
#define AV_SLOTS_FOR_CONNS(nconns) ((nconns) / 6)
   ...
   *
   * 在探测 shared_buffers 之前先探测 max_connections，
   * 因为它受到的限制比 shared_buffers 更多。我们还在
   * 这里选择默认的 autovacuum_worker_slots。
   */
...
static bool test_specific_config_settings(int test_conns, int test_av_slots,
  ...
  printfPQExpBuffer(&cmd,
                    ...
                    backend_exec, boot_options, extra_options, test_conns,
                    test_av_slots, test_buffs, dynamic_shared_memory_type);
...
static void setup_config(void) {
  ...
  conflines =
      replace_guc_value(conflines, "autovacuum_worker_slots", repltok, false);
  ...
  if (default_timezone) {
    conflines =
        replace_guc_value(conflines, "timezone", default_timezone, false);
    conflines =
        replace_guc_value(conflines, "log_timezone", default_timezone, false);
  }
  ...
  /* 注意：这些取决于 wal_segment_size_mb，它们不是常量 */
   ...
   *
   * 修复各种条目以匹配真实的编译时默认值。由于这些确实
   * 是默认值，请保留 postgresql.conf 行的注释。
   */
  conflines = replace_guc_value(conflines, "unix_socket_directories",
                                DEFAULT_PGSOCKET_DIR, true);

  conflines = replace_guc_value(conflines, "port", DEF_PGPORT_STR, true);

#if DEFAULT_BACKEND_FLUSH_AFTER > 0
  snprintf(repltok, sizeof(repltok), "%dkB",
           DEFAULT_BACKEND_FLUSH_AFTER * (BLCKSZ / 1024));
  conflines =
      replace_guc_value(conflines, "backend_flush_after", repltok, true);
#endif
  ...
  conflines =
      replace_guc_value(conflines, "bgwriter_flush_after", repltok, true);
#endif
  ...
  conflines =
      replace_guc_value(conflines, "checkpoint_flush_after", repltok, true);
#endif

#ifdef WIN32
  conflines = replace_guc_value(conflines, "update_process_title", "off", true);
#endif
   ...
   *
   * 如果选择了 md5 作为身份验证方法，除非也选择了 scram-sha-256，
   * 否则将 password_encryption 设置更改为 md5。
   */
  if ((strcmp(authmethodlocal, "md5") == 0 &&
    ...
    conflines =
        replace_guc_value(conflines, "password_encryption", "md5", false);
  }
   ...
   * relocated.
   *
   * 如果已为集群启用组访问权限，那么确保日志文件也允许组访问权限
   * 是有意义的。否则，如果日志文件未重新定位，组中的用户进行
   * 备份将会失败。
   */
  if (pg_dir_create_mode == PG_DIR_MODE_GROUP) {
    conflines = replace_guc_value(conflines, "log_file_mode", "0640", false);
  }
   ...
   *
   * 探测是否真的有平台支持 IPv6，如果没有，则注释掉相关的 
   * pg_hba 行。如果 getaddrinfo 实际上无法处理 IPv6，这可以
   * 避免运行时警告。这在 Windows 上特别有用，因为在具有 
   * IPv6 的机器上构建的可执行文件可能必须在没有 IPv6 的机器上运行。
   */
  {
...
static void bootstrap_template1(void) {
  ...
  if (strcmp(headerline, *bki_lines) != 0) {
    pg_log_error("input file \"%s\" does not belong to PostgreSQL %s", bki_file,
                 PG_VERSION);
  ...
  bki_lines =
      replace_token(bki_lines, "ENCODING", encodingid_to_string(encodingid));

  bki_lines =
      replace_token(bki_lines, "LC_COLLATE", escape_quotes_bki(lc_collate));

  bki_lines = replace_token(bki_lines, "LC_CTYPE", escape_quotes_bki(lc_ctype));

  bki_lines =
      replace_token(bki_lines, "DATLOCALE",
  ...
  printfPQExpBuffer(&cmd, "\"%s\" --boot %s %s", backend_exec, boot_options,
                    extra_options);
...
static void setup_privileges(FILE *cmdfd) {
  PG_CMD_PRINTF(
      ...
      "    CASE WHEN relkind = " CppAsString2(
          RELKIND_SEQUENCE) " THEN 's' "
                            "         ELSE 'r' END::\"char\"," CppAsString2(
                                BOOTSTRAP_SUPERUSERID) "::oid))"
                                                       " ) as a) "
                                                       "  WHERE relkind IN "
                                                       "(" CppAsString2(RELKIND_RELATION) ", " CppAsString2(RELKIND_VIEW) ", " CppAsString2(
                                                           RELKIND_MATVIEW) ","
  ...
  PG_CMD_PUTS(
      ...
      "    WHERE"
      "        relacl IS NOT NULL"
      "        AND relkind IN (" CppAsString2(
          RELKIND_RELATION) ", " CppAsString2(RELKIND_VIEW) ", " CppAsString2(RELKIND_MATVIEW) ", " CppAsString2(RELKIND_SEQUENCE) ");\n\n");
  PG_CMD_PUTS("INSERT INTO pg_init_privs "
              ...
              "        AND pg_class.relkind IN (" CppAsString2(RELKIND_RELATION) ", " CppAsString2(
                  RELKIND_VIEW) ", " CppAsString2(RELKIND_MATVIEW) ","
                                                                   " " CppAsString2(RELKIND_SEQUENCE) ");\n\n");
  PG_CMD_PUTS("INSERT INTO pg_init_privs "
              "  (objoid, classoid, objsubid, initprivs, privtype)"
              "    SELECT"
 ...
 *
 * 位于 https://docs.microsoft.com/en-us/cpp/c-runtime-library/reference/signal 
 * 的 Windows 运行时文档明确禁止从信号处理程序执行许多操作，
 * 包括 I/O、内存分配和系统调用，并且仅在处理 SIGFPE 时才
 * 允许 jmpbuf。
 *
 * I avoided doing the forbidden things by setting a flag instead of calling
 * exit() directly.
 *
 * 我通过设置标志而不是直接调用 exit() 来避免执行被禁的操作。
 *
 ...
 *
 * 另外请注意 Windows 对 SIGINT 的行为，其说明如下：任何 Win32 
 * 应用程序都不支持 SIGINT。当发生 CTRL+C 中断时，Win32 操作
 * 系统会生成一个新线程专门处理该中断。这会导致单线程应用
 * 程序（如 UNIX 中的应用程序）变成多线程并引起意外行为。
 *
 * I have no idea how to handle this. (Strange they call UNIX an application!)
 * So this will need some testing on Windows.
 *
 * 我不知道如何处理这个。 (奇怪的是他们把 UNIX 称为一个应用程序！)
 * 所以这需要在 Windows 上进行一些测试。
 */
static void trapsig(SIGNAL_ARGS) {
  /* handle systems that reset the handler, like Windows (grr) */
  /* 处理重置处理程序的系统，例如 Windows (grr) */
 ...
 *
 * 如果成功，且 canonname 不为 NULL，则在其中存储该区域设置规范
 * 名称的 malloc 副本。这对于弄清楚区域设置名称 "" 的含义
 * （即环境值）特别有用。（实际上，在大多数实现中，这是它唯一
 * 的好处；我们希望 setlocale 能返回区域设置名称的规范拼写版本，
 * 但通常它不返回。）
 *
 * this should match the backend's check_locale() function
 *
 * 这应该与后端的 check_locale() 函数相匹配。
 */
static void check_locale_name(int category, const char *locale,
                              char **canonname) {
  save_locale_t save;
  ...
  if (res == NULL) {
    if (*locale) {
      ...
      pg_log_error_hint(
          "If the locale name is specific to ICU, use --icu-locale.");
      exit(1);
    } else {
      /*
...
static bool check_icu_locale_encoding(int user_enc) {
  if (!(is_encoding_supported_by_icu(user_enc))) {
    pg_log_error("encoding mismatch");
    pg_log_error_detail("The encoding you selected (%s) is not supported with "
                        "the ICU provider.",
...
static void icu_validate_locale(const char *loc_str) {
  ...
  if (U_FAILURE(status)) {
    pg_fatal("could not get language from locale \"%s\": %s", loc_str,
             u_errorName(status));
    return;
  }
  ...
  /* 检查特殊的语言名称 */
  if (strcmp(lang, "") == 0 || strcmp(lang, "root") == 0 ||
      strcmp(lang, "und") == 0)
    found = true;
  ...
  if (!found)
    pg_fatal("locale \"%s\" has unknown language \"%s\"", loc_str, lang);
#else
  pg_fatal("ICU is not supported in this build");
#endif
}
...
static void setlocales(void) {
  ...
  /* 如果已设置区域设置配置，则将空的 lc_* 和 datlocale 值设置为该配置 */
  ...
  /* 当 LC_MESSAGES 不可用时，使用 LC_CTYPE 设置 */
  ...
  if (locale_provider == COLLPROVIDER_BUILTIN) {
     ...
     *
     * 在支持的构建中，ICU 区域设置 ID 将在引导后的初始化期间
     * 打开，届时将执行额外检查。
     */
#ifndef USE_ICU
    pg_fatal("ICU is not supported in this build");
...
static void usage(const char *progname) {
  ...
  printf(_("  -g, --allow-group-access  allow group read/execute on data "
           "directory\n"));
  printf(
      _("      --icu-locale=LOCALE   set ICU locale ID for new databases\n"));
  printf(_("      --icu-rules=RULES     set additional ICU collation rules for "
           "new databases\n"));
  ...
  printf(_("      --builtin-locale=LOCALE\n"
           "                            set builtin locale name for new "
           "databases\n"));
  printf(_("      --locale-provider={builtin|libc|icu}\n"
           "                            set default locale provider for new "
           "databases\n"));
  printf(_("      --no-data-checksums   do not use data page checksums\n"));
  printf(_("      --pwfile=FILE         read password for the new superuser "
           "from file\n"));
  printf(_("  -T, --text-search-config=CFG\n"
           "                            default text search configuration\n"));
  printf(_("  -U, --username=NAME       database superuser name\n"));
  printf(_("  -W, --pwprompt            prompt for a password for the new "
           "superuser\n"));
  printf(_("  -X, --waldir=WALDIR       location for the write-ahead log "
           "directory\n"));
  printf(_("      --wal-segsize=SIZE    size of WAL segments, in megabytes\n"));
  printf(_("\nLess commonly used options:\n"));
  printf(_("  -c, --set NAME=VALUE      override default setting for server "
           "parameter\n"));
  ...
  printf(_("      --no-sync-data-files  do not sync files within database "
           "directories\n"));
  printf(_("      --no-instructions     do not print instructions for next "
           "steps\n"));
  printf(_("  -s, --show                show internal settings, then exit\n"));
  printf(
      _("      --sync-method=METHOD  set method for syncing files to disk\n"));
  printf(_("  -S, --sync-only           only sync database files to disk, then "
           "exit\n"));
  printf(_("\nOther options:\n"));
  printf(
...
static void check_need_password(const char *authmethodlocal,
  ...
  if ((strcmp(authmethodlocal, "md5") == 0 ||
      ...
      (strcmp(authmethodhost, "md5") == 0 ||
       ...
       strcmp(authmethodhost, "scram-sha-256") == 0) &&
      !(pwprompt || pwfilename))
    pg_fatal("must specify a password for the superuser to enable password "
             "authentication");
}
...
void setup_pgdata(void) {
  ...
  if (!pg_data) {
    ...
    if (pgdata_get_env && strlen(pgdata_get_env)) {
      ...
      pg_log_error_hint(
          "You must identify the directory where the data for this database "
          "system "
...
void setup_bin_paths(const char *argv0) {
  ...
  if ((ret = find_other_exec(argv0, "postgres", PG_BACKEND_VERSIONSTR,
    ...
    if (ret == -1)
      pg_fatal("program \"%s\" is needed by %s but was not found in the same "
               "directory as \"%s\"",
               "postgres", progname, full_path);
    else
      pg_fatal("program \"%s\" was found by \"%s\" but was not the same "
               "version as %s",
...
void setup_locale_encoding(void) {
  ...
  if (locale_provider == COLLPROVIDER_LIBC &&
      strcmp(lc_ctype, lc_collate) == 0 && strcmp(lc_ctype, lc_time) == 0 &&
    ...
    printf(_("The database cluster will be initialized with this locale "
             "configuration:\n"));
...
void setup_text_search(void) {
  if (!default_text_search_config) {
    ...
    if (checkmatch == NULL) {
      ...
      pg_log_warning("specified text search configuration \"%s\" might not "
                     "match locale \"%s\"",
  ...
  if (xlog_dir) {
    ...
    case 4:
      ...
      if (ret != 4)
        ...
        pg_log_error_hint("If you want to store the WAL there, either remove "
                          "or empty the directory \"%s\".",
                          xlog_dir);
      exit(1);
...
void warn_on_mount_point(int error) {
  if (error == 2)
    pg_log_error_detail("It contains a dot-prefixed/invisible file, perhaps "
                        "due to it being a mount point.");
  else if (error == 3)
    pg_log_error_detail("It contains a lost+found directory, perhaps due to it "
                        "being a mount point.");
...
void initialize_data_directory(void) {
   ...
   *
   * 根据请求的 PGDATA 权限设置掩码。pg_mode_mask 以及像
   * pg_dir_create_mode 这样的相关项默认设置为仅所有者，如果在
   * 解析选项时传递了 -g，则通过调用 SetDataDirectoryCreatePerm()
   * （见上文）进行更新。
   */
  umask(pg_mode_mask);
  ...
  for (i = 0; i < lengthof(subdirs); i++) {
     ...
     *
     * 父目录已经存在，所以这里我们只需要 mkdir() 而不需要
     * pg_mkdir_p()，这可以避免一些失败模式；参见 bug #13853。
     */
   ...
   *
   * 仅在初始化 template1 后才为其创建每个数据库的 PG_VERSION
   */
   ...
   *
   * 使用以简单独立模式运行的后端，创建我们不需要使用引导模式的内容。
   */
  fputs(_("performing post-bootstrap initialization ... "), stdout);
  fflush(stdout);

  initPQExpBuffer(&cmd);
  printfPQExpBuffer(&cmd, "\"%s\" %s %s template1 >%s", backend_exec,
   ...
   *
   * 请注意，在 setup_depend() 之后创建的对象都不会被 "固定"。
   * DBA 可以随意删除它们。
   */
...
int main(int argc, char *argv[]) {
  /* long_options */
  /*
   * long_options 数组定义了 initdb 命令支持的所有长选项。
   * 每个选项都有一个名称、一个标志和一个值。
   */
   ...
   *
   * 确保 stdout 的缓冲行为与交互式用法（至少在大多数平台上）相匹配。
   * 这可以防止在（例如）输出重定向到文件时出现意外的输出顺序。
   * POSIX 说我们必须在这些文件的任何其他用法之前执行此操作。
   */
  ...
  while ((c = getopt_long(argc, argv, "A:c:dD:E:gkL:nNsST:U:WX:", long_options,
                          &option_index)) != -1) {
    switch (c) {
    case 'A':
    ...
    case 'n':
      noclean = true;
      printf(
          _("Running in no-clean mode.  Mistakes will not be cleaned up.\n"));
      break;
    case 'N':
      do_sync = false;
      break;
    ...
    case 'X':
      ...
      break;
    case 12:
      if (!option_parse_int(optarg, "--wal-segsize", 1, 1024,
                            &wal_segment_size_mb))
        exit(1);
      break;
    case 13:
    ...
    case 14:
      extra_options =
          psprintf("%s %s", extra_options, "-c debug_discard_caches=1");
      break;
    case 15:
   ...
   *
   * 非选项参数指定数据目录，只要它还没有用 -D / --pgdata 指定
   */
  ...
  if (!IsValidWalSegSize(wal_segment_size_mb * 1024 * 1024))
    pg_fatal("argument of %s must be a power of two between 1 and 1024",
             "--wal-segsize");
  ...
  if (strncmp(username, "pg_", 3) == 0)
    pg_fatal("superuser name \"%s\" is disallowed; role names cannot begin "
             "with \"pg_\"",
             username);
